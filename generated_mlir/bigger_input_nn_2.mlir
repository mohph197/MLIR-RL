...
module attributes {torch.debug_module_name = "Net"} {
  ...
  func.func @forward() -> tensor<32x10xf32> {
    ...
    %cst_1 = bufferization.alloc_tensor() : tensor<32x3x242x242xf32>
    %cst_2 = bufferization.alloc_tensor() : tensor<64x3x7x7xf32>
    %cst_22 = bufferization.alloc_tensor() : tensor<3x64x7x7xf32>
    %cst_33 = bufferization.alloc_tensor() : tensor<3xf32>
    ...
    %a0 = tensor.empty() : tensor<32x64x236x236xf32>
    %a1 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%cst_3 : tensor<64xf32>) outs(%a0 : tensor<32x64x236x236xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<32x64x236x236xf32>
    %a2 = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>} ins(%cst_1, %cst_2 : tensor<32x3x242x242xf32>, tensor<64x3x7x7xf32>) outs(%a1 : tensor<32x64x236x236xf32>) -> tensor<32x64x236x236xf32>
    %a3 = linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%a2 : tensor<32x64x236x236xf32>) outs(%a0 : tensor<32x64x236x236xf32>) {
    ^bb0(%in: f32, %out: f32):
    ...
    %b0 = tensor.empty() : tensor<32x3x230x230xf32>
    %b1 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%cst_33 : tensor<3xf32>) outs(%b0 : tensor<32x3x230x230xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<32x3x230x230xf32>
    %b2 = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>} ins(%a3, %cst_22 : tensor<32x64x236x236xf32>, tensor<3x64x7x7xf32>) outs(%b1 : tensor<32x3x230x230xf32>) -> tensor<32x3x230x230xf32>
    %b3 = linalg.generic {indexing_maps = [#map1, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%b2 : tensor<32x3x230x230xf32>) outs(%b0 : tensor<32x3x230x230xf32>) {
    ^bb0(%in: f32, %out: f32):
      %cst_00 = arith.constant 0.000000e+00 : f32
      %35 = arith.cmpf ugt, %in, %cst_00 : f32
      %36 = arith.select %35, %in, %cst_00 : f32
    ...
    %2 = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<2> : vector<2xi64>} ins(%b3, %cst_2 : tensor<32x3x230x230xf32>, tensor<64x3x7x7xf32>) outs(%1 : tensor<32x64x112x112xf32>) -> tensor<32x64x112x112xf32>
